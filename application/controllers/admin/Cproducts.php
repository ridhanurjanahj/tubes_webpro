...
class Cproducts extends CI_Controller
{
    ...
    {
        parent::__construct();
        $this->load->model("product_model"); //Pada method ini, kita melakukan load model (porduct_model) dan library (form_validation).
    ...
    {
        $data["products"] = $this->product_model->getAll(); //Pada method ini, kita akan mengambil data dari model dengan memanggil method product_model->getAll().
        $this->load->view("admin/product/list", $data); //Lalu kita me-rendernya ke dalam view admin/product/list.
    }
    ...
    public function add()
    {
        ...
        $this->load->view("admin/product/new_form"); // tamilkan form add
    }
    ...
    public function edit($id = null)
    {
        if (!isset($id)) redirect('admin/cproducts'); // redirect ke route ini kalau $id bernilai null / redirect juga tidak ada $id
        ...
        $data["product"] = $product->getById($id);      // mngembil data untuk di tampilkan pada form
        if (!$data["product"]) show_404();              //if  tidak ada data, tampilan error 404
    ...
    public function delete($id=null)
    {
        ...
        if ($this->product_model->delete($id)) {
            redirect(site_url('admin/cproducts')); //Apabila data berhasil dihapus, maka kita langsung alihkan (redirect()) menuju ke halaman admin/products/.
        }
    }
}