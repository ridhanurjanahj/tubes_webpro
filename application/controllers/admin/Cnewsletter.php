...
class Cnewsletter extends CI_Controller
{
    ...
    {
        parent::__construct();
        $this->load->model("newsletter_model"); //Pada method ini, kita melakukan load model (porduct_model) dan library (form_validation).
    ...
    {
        $data["newsletter"] = $this->newsletter_model->getAll(); //Pada method ini, kita akan mengambil data dari model dengan memanggil method product_model->getAll().
        $this->load->view("admin/newsletter/list", $data); //Lalu kita me-rendernya ke dalam view admin/product/list.
    }
    ...
    public function add()
    {
        $product = $this->newsletter_model;    //objek model
        ...
        $this->load->view("home"); // tamilkan form add
    }
    ...
    public function edit($id = null)
    {
        if (!isset($id)) redirect('admin/cnewsletter'); // redirect ke route ini kalau $id bernilai null / redirect juga tidak ada $id
        ...
        $data["newsletter"] = $product->getById($id);      // mngembil data untuk di tampilkan pada form
        if (!$data["newsletter"]) show_404();              //if  tidak ada data, tampilan error 404

        $this->load->view("admin/newsletter/edit_form", $data);    //menampilkan form edit
    }
    ...
    public function delete($id=null)
    {
        ...
        if ($this->newsletter_model->delete($id)) {
            redirect(site_url('admin/cnewsletter')); //Apabila data berhasil dihapus, maka kita langsung alihkan (redirect()) menuju ke halaman admin/products/.
        }
    }
}